...
from abilene_dataset import LoadAbileneData
import numpy as np
...
class GCN(nn.Module):
    def __init__(self,in_c,hid_c,out_c,device):
        ...
        self.my_w=torch.randn(12,12,2,requires_grad=True)
        self.device=device
        # print(self.my_w)
...
class FFNN(nn.Module):
    ...
    def forward(self,data,adj,is_training=False):
        flow_x=data["flow_x"][:,:,:,-1].unsqueeze(-1).to(self.device)
        B,N,H,D=flow_x.size()
        flow_x=flow_x.view(B,N,-1)
        output=self.layer(flow_x)
...
class LSTM_FFNN(nn.Module):
    def __init__(self,in_c,out_c,device):
        ...
        self.lstm=LSTM_TM(in_dim=144, hidden_dim=144)
        self.ffnn=FFNN(in_c=12, out_c=12,device=device)
        self.device=device
...
def print_images(true_value,pred_value):
    ...
    for i in range(12):
        for j in range(12):
            ...
            plt.legend()
            plt.title("node:" + str(i*12+j))
            f = plt.gcf()  # 获取当前图像
            f.savefig("./images/diff_gcn/node"  + str(i*12+j) + ".png")
            # plt.show()
            f.clear()  # 释放内存

def predict_data(model,print_log,device,criterion,args,pred_n=1):
    if pred_n == 1:
        test_start_time = time.time()
    test_data = LoadAbileneData(data_path=["./simple/abilene_a_metrix.csv", "./simple/abilene_norm.npz"], num_nodes=12,
                              divide_days=[45, 14],
...
def main(model_name,epochs,n_his=16,model=None,model_n=None,is_slx=False,is_long_term=False):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    args,blocks=get_parameters()
    args.model=model_name
    args.n_his=n_his
    # if model_name=="MyGAT3_1" or model_name=="GAT" or model_name=='ChebNet' or model_name=='ResGAT':
    #     args.n_his=1
    args.epochs=epochs
    ...
    if args.model == 'cheb_graph_conv':
        my_net = STGCNChebGraphConv(args, blocks, 144,device)
    elif args.model=='STM':
        my_net=model
    elif args.model=='CNN_T':
        my_net=CNN_T(device)
    elif args.model=='CNN_S':
        my_net=CNN_S(12,12,device)
    elif args.model=='FFNN_T':
        my_net=FFNN_T(args.n_his,1,device)
    elif args.model == 'graph_conv':
        my_net = STGCNGraphConv(args, blocks, 144,device)
    elif args.model=='FFNN':
        my_net = FFNN(in_c=12, out_c=12,device=device)
    elif args.model=='LSTM_FFNN':
        my_net = LSTM_FFNN(in_c=12, out_c=12,device=device)
    elif args.model=='Lstm_tm':
        my_net=LSTM_TM(in_dim=144, hidden_dim=144)
        ...
        my_net=GCN(in_c=12*1,hid_c=12*1,out_c=12*1,device=device)
    elif args.model=='ChebNet':
        my_net=ChebNet(in_c=12,hid_c=12,out_c=12,K=2,device=device)
    elif args.model=='GAT':
        my_net = GAT(nfeat=12,
                    nhid=12,
                    nclass=12,
    ...
    optimizer = optim.Adam(params=my_net.parameters(),lr=0.0011)
    ...
    for epoch in range(args.epochs):
        if is_slx==True and (epoch==0 or epoch==1 or epoch%5==0):
            predict_data(my_net, print_log, device, criterion, args, pred_n=1)
        my_net.train()
        epoch_loss=0.0
        ...
        print("Epoch:{:04d},Loss:{:.4f},Time:{:02.2f} mins".format(epoch, 10000 * epoch_loss / len(train_data),
                                                                   (end_time - start_time) / 60),file=print_log)
...
def print_single_prediction(epochs=50):
    ...
    # main("Lstm", epochs, n_his=16,is_long_term=True)
    # print("====================end LSTM-FBF 模型预测============================")
    # print("==================start GRU-FBF 模型预测===========================")
    # main("GRU", epochs, n_his=16,is_long_term=True)
    # print("====================end GRU-FBF 模型预测============================")
    # print("==================start GCN(1st) 模型预测===========================")
    # main("GCN", epochs, n_his=1,is_long_term=False)
    # print("====================end GCN(1st) 模型预测============================")
    # print("==================start GCN(Cheb) 模型预测===========================")
    # main("ChebNet", epochs, n_his=1,is_long_term=False)
    # print("====================end GCN(Cheb) 模型预测============================")
    print("==================start GAT 模型预测===========================")
    main("GAT", epochs, n_his=1,is_long_term=False)
    print("====================end GAT 模型预测============================")
    print("==================start GSASN 模型预测===========================")
    main("ResGAT", epochs, n_his=1,is_slx=True,is_long_term=False)
    print("====================end GSASN 模型预测============================")
    print("==================start LSTM-FFNN 模型预测===========================")
    main("LSTM_FFNN", epochs, n_his=16,is_long_term=False)
    print("====================end LSTM-FFNN 模型预测============================")
    print("==================start STGCN(Cheb) 模型预测===========================")
    main("cheb_graph_conv", epochs, n_his=16,is_long_term=False)
    print("====================end STGCN(Cheb) 模型预测============================")
    print("==================start STGCN(1st) 模型预测===========================")
    main("graph_conv", epochs, n_his=16,is_long_term=False)
    print("====================end STGCN(1st) 模型预测============================")
    print("==================start FFNN-GSASN 模型预测===========================")
    print_models_zh1(["FFNN_T"],["ResGAT"],epochs,is_long_term=False)
    print("====================end FFNN-GSASN 模型预测============================")
    print("==================start CNN-GSASN 模型预测===========================")
    print_models_zh1(["CNN_T"], ["ResGAT"], epochs,is_long_term=False)
    print("====================end CNN-GSASN 模型预测============================")
    print("==================start GRU-GSASN 模型预测===========================")
    print_models_zh1(["GRU"], ["ResGAT"], epochs,is_slx=True,is_long_term=False)
...
def print_sequence_length(epochs=50):
    ...
    for l in sq_len:
        print("=============sq_len ", l, "===============")
        main("graph_conv", epochs, n_his=16)
        print("======================================")
    print("====================end STGCN(1st) 模型预测============================")
    print("==================start LSTM-FFNN 模型预测===========================")
    for l in sq_len:
        print("=============sq_len ", l, "===============")
        main("LSTM_FFNN", epochs, n_his=16)
        print("======================================")
    print("====================end LSTM-FFNN 模型预测============================")
    print("==================start LSTM 模型预测===========================")
    for l in sq_len:
        print("=============sq_len ", l, "===============")
        main("Lstm_tm", epochs, n_his=16)
        print("======================================")
    print("====================end LSTM 模型预测============================")
        ...
        for tm_name in time_models_name:
            for sp_name in space_models_name:
                ...
                if sp_name == 'CNN_S':
                    ...
                    sp = GCN(in_c=12 * 1, hid_c=12 * 1, out_c=12 * 1, device=device)
                elif sp_name == 'ChebNet':
                    sp = ChebNet(in_c=12, hid_c=12, out_c=12, K=2, device=device)
                elif sp_name == 'CNN_S':
                    sp = CNN_S(12, 12, device)
                elif sp_name == 'ResGAT':
                    sp = ResGAT(nfeat=12,
...
def print_models_zh(epochs = 50):
    ...
    for tm_name in time_models_name:
        for sp_name in space_models_name:
            ...
            for l in sq_len:
                print("=============sq_len ", l, "===============")
                main("STM",epochs=epochs,n_his=l,model=stm,model_n=tm_name+"-"+sp_name)
                print("======================================")
            print("=============end", tm_name, "-", sp_name, "===============")
...
def print_st_models(epochs=50):
    ...
    # print_models_zh1(["Lstm", "FFNN_T", "CNN_T", "Lstm_tm"], ["ChebNet", "ResGAT", "GCN", "CNN_S", "GAT"],epochs=1)
if __name__=='__main__':
    print_single_prediction(epochs=50)  # 打印短长期预测
    print_model_effect(epochs=50) #打印模型各个模块有效性
    print_sequence_length(epochs=50) #打印各个模型随序列长度的变化
    print_slx(epochs=50) #打印收敛性
    print_st_models(epochs=50) #打印各个时间和空间组合性能比较
