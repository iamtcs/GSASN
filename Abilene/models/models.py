...
class MyGAT(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads,device):
        ...
        self.device=device
        w=torch.rand(12, 12,12).float()
        b = torch.rand(12, 12, 1).float()
        # for i in range(12):
        #     for j in range(12):
        #         w[i,j,j]=1
    ...
    def forward(self, data, adj,is_training=True):
        ...
        for j in range(B):
            x[j] = torch.cat([attention[i] @ (data[j, :, i].unsqueeze(-1)) for i in range(12)], dim=1).t()
        # cj=(x-data)/data
...
class ResGAT(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads,device):
        ...
        self.gat = MyGAT3_1(nfeat=12,
                         ...
                         alpha=0.2,
                         device=device)
        self.a = nn.Parameter(torch.rand(1,12,12, 2).float())
...
class ResGATByNum(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads,res_gat_num,device):
        ...
        self.resgats = [ResGAT(nfeat=12,nhid=12,nclass=12,dropout=0.1,nheads=1,alpha=0.2,device=device) for _ in range(res_gat_num)]
...
class ResGAT1ByNum(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads,res_gat_num,device):
        ...
        self.resgats = [ResGAT1(nfeat=12,nhid=12,nclass=12,dropout=0.1,nheads=1,alpha=0.2,device=device) for _ in range(res_gat_num)]
        for i, resgat in enumerate(self.resgats):
            self.add_module('resgat_{}'.format(i), resgat)
...
class MyGAT2(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads,device):
        ...
        self.dropout = dropout
        self.device = device
        self.W1 = nn.Parameter(torch.rand(12, 12,12).float())
        nn.init.xavier_uniform_(self.W1.data, gain=1.414)
        self.W2 = nn.Parameter(torch.rand(12, 12, 12).float())
    ...
    def forward(self, data, adj,is_training=True):
        ...
        # attention=torch.where(torch.tensor(adj)>0,attention,0)
        # attention = attention / attention.sum(dim=-1, keepdim=True)
        x1=torch.cat([attention1[i]@(data[:,i].unsqueeze(-1)) for i in range(12)],dim=1)
        data=data.t()
        ...
        x2 = torch.cat([attention2[i] @ (data[:, i].unsqueeze(-1)) for i in range(12)], dim=1).t()
        x=(x1+x2)/2
        # x=data["flow_x"].squeeze(-1).squeeze(0)
        # x = F.dropout(x, self.dropout, training=self.training)
        # x = torch.cat([att(x, adj,is_training) for att in self.attentions], dim=-1)  # 将每层attention拼接
...
class MyGAT3_1(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads,device):
        ...
        self.gat1 = MyGAT1(nfeat=12,
                          ...
                          alpha=0.2,
                          device=device)
        self.a = nn.Parameter(torch.rand(12,12,2).float())
