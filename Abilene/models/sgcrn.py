...
class GCN(nn.Module):
    ...
    def forward(self,data,adj,is_training=False):
        ...
        # graph_data=graph_data.unsqueeze(0).expand(64,12,12)
...
class SGCRU(nn.Module):
    def __init__(self, in_dim, hidden_dim,device, n_layer=1, seq_len=12, pre_len=1):
        ...
        self.gcn=ChebNet(in_c=12,hid_c=12,out_c=12,K=2,device=device)
    ...
    def forward(self, data,adj,is_training=True):
        ...
        result=self.gcn({"flow_x":ouput_n.reshape(-1,12,12,1),"graph":data["graph"]},adj,is_training=True)
        # for i in range(12):
        #     for j in range(12):
