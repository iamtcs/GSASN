...
class LSTM(nn.Module):
    ...
    def forward(self, data,adj,is_training=True):
        ...
        return ouput_hn.reshape(-1, 12, 12, 1)
...
class LSTM_GAT(nn.Module):
    ...
    def forward(self, data,adj,is_training=True):
        ...
        #         ouput, ouput_n = self.rnn(x[:,i,j].unsqueeze(-1))  # BS,H,1
        #         result = torch.cat([result,ouput_n],dim=-1)
        lstm_output=ouput_hn.reshape(-1, 12, 12, 1)
...
class RNN(nn.Module):
    ...
    def forward(self, data,adj,is_training=True):
        ...
        #         ouput, ouput_n = self.rnn(x[:,i,j].unsqueeze(-1))  # BS,H,1
        #         result = torch.cat([result,ouput_n],dim=-1)
        return ouput_n.reshape(-1,12,12,1)
...
class GRU(nn.Module):
    ...
    def forward(self, data,adj,is_training=True):
        ...
        #         ouput, ouput_n = self.rnn(x[:,i,j].unsqueeze(-1))  # BS,H,1
        #         result = torch.cat([result,ouput_n],dim=-1)
        return ouput_n.reshape(-1,N,N,1)