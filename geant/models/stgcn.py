...
class STGCNChebGraphConv(nn.Module):
    ...
    def __init__(self, args, blocks, n_vertex,device):
    ...
    def forward(self, x,adj,is_training=False):
        x = x["flow_x"]
        B,N,N,H=x.shape
        x=x.view(B,-1,H).permute(0,2,1).unsqueeze(1).to(self.device)
        x = self.st_blocks(x)
        if self.Ko > 1:
            x = self.output(x)
        elif self.Ko == 0:
