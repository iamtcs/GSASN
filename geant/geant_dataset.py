...
class LoadGeantData(Dataset):
    ...
    def __getitem__(self,index):
        ...
        data_x,data_y=LoadGeantData.slice_data(self.flow_data,self.history_length,index,self.predict_length,self.train_mode)
        data_x=LoadGeantData.to_tensor(data_x)
        data_y=LoadGeantData.to_tensor(data_y)

        return {"graph":LoadGeantData.to_tensor(self.graph),"flow_x":data_x,"flow_y":data_y}
    ...
    def pre_process_data(data,norm_dim):
        norm_base=LoadGeantData.normalize_base(data,norm_dim)
        norm_data=LoadGeantData.normalize_data(norm_base[0],norm_base[1],data)
...
if __name__=='__main__':
    ...
    for data in test_loader:
        recovered_true = LoadGeantData.recover_data(max_data=train_data.flow_norm[0], min_data=train_data.flow_norm[1],
                                               data=data["flow_y"])
